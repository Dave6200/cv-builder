      ...
      <div style={{
        ...
        <div style={{
          ...
          {data.photo ? (
            ...
            <span style={{ fontSize: '26px', fontWeight: '400', color: gold, fontFamily: 'Georgia, serif' }}>
              {getInitials(data.name)}
            </span>
          )}
        </div>

        {/* Декоративные линии с ромбом */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '14px' }}>
          <div style={{ width: '56px', height: '1px', background: gold }} />
          <span style={{ color: gold, fontSize: '8px', letterSpacing: '2px' }}>✦</span>
          <div style={{ width: '56px', height: '1px', background: gold }} />
        </div>
        ...
        {/* Контакты по центру с точками-разделителями */}
        {contacts.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            {contacts.map((item, i) => (
              <span key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {i > 0 && <span style={{ color: gold, fontSize: '8px' }}>✦</span>}
